    ...
    <div style={{ padding: '40px', fontFamily: 'sans-serif', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '24px', marginTop: '24px' }}>
        {mockEvents.map((event) => (
          <div key={event.id} style={{ padding: '24px', border: '1px solid #e2e8f0', borderRadius: '16px', backgroundColor: 'white' }}>
            <h3 style={{ margin: '0 0 8px 0', color: '#1e293b', fontSize: '1.25rem', fontWeight: '700' }}>{event.title}</h3>
            <p style={{ margin: '4px 0', color: '#64748b', fontSize: '0.9rem' }}>📍 {event.venue} — {event.city}</p>
            <p style={{ fontWeight: '800', color: '#0f172a', fontSize: '1.35rem', marginTop: '15px' }}>{event.currency}{event.minPrice}</p>
          </div>
        ))}
      </div>
    </div>
